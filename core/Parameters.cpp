...
Parameters::Parameters()
{
	...
	auto fnCreateParameter= [&](const ParameterName name, const float initValue, const std::pair<float, float>& range, const std::pair<ParameterBehaviour, float>& behaviour) {
	...
	fnCreateParameter(ParameterName::T,			300.0f,	{ 75.f, 600.0f }, { ParameterBehaviour::Pitch, 4.f});
...
Parameter::Parameter(const ParameterName name, const float value, const std::pair<float, float>& range,
	const std::pair<ParameterBehaviour, float>& behaviour) : m_name(name), m_value(value), m_range(range), m_behaviour(behaviour.first), m_pitchRatio(behaviour.second)
{
